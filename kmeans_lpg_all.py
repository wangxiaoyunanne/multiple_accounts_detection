...
#bipart = pd.read_table('sample_file/pop_name/name0000000000000100_00009242.txt', sep = ' ', header = None)

#df_bipart = pd.read_table('sample_file/emb/name100.emb', sep = ' ', header = None) 
...
#truth = pd.read_table('sample_file/truth_file/truth00000000100_00009242.txt', sep = ',')

#subGraph = nx.read_weighted_edgelist('sample_file/graphs/graph100_00009242edgelist.gz')
# read all files and sort them
bipart_files = glob.glob('sample_file/pop_name/*.txt')
bipart_files.sort()
df_bipart_files = glob.glob('sample_file/emb/*.emb')
df_bipart_files.sort()
kTruth_files = glob.glob('sample_file/kTruth/*.txt')
kTruth_files.sort()
truth_files = glob.glob('sample_file/truth_file/*.txt')
truth_files.sort()
subGraph_files = glob.glob('sample_file/graphs/*.gz')
subGraph_files.sort()
...
file_indices = range(len(truth_files))

for f_index in file_indices[5:10] :
    random.seed(7)
...
# a subset of X_id [] 
    X_IDs= []
# read files
    bipart = pd.read_table(bipart_files[f_index], sep = ' ', header = None)
    df_bipart = pd.read_table(df_bipart_files[f_index], sep = ' ', header = None)
# kTruth column '0' and '1'
    kTruth = pd.read_table(kTruth_files[f_index], sep = ',')
# get ground truth and sample some data
#truth = pd.read_table('ground_truth/truth_49070.txt', sep = ',')
    truth = pd.read_table(truth_files[f_index], sep = ',')
    subGraph = nx.read_weighted_edgelist(subGraph_files[f_index])
# finished of reading files
...
########################build semi-supervied model #########################
    ...
    result_file = 'res_f1_p_r/all/' + bipart_files[f_index][30:]
    f1_w = f1_score(y_whole , y_fitted_whole, average = 'weighted' )
    presi_w = precision_score(y_whole ,y_fitted_whole, average = 'weighted')  
    recall_w = recall_score(y_whole , y_fitted_whole, average = 'weighted')

    f1_n = f1_score(y_whole , y_fitted_whole )
    presi_n = precision_score(y_whole ,y_fitted_whole)
    recall_n = recall_score(y_whole , y_fitted_whole)

    f1_b = f1_score(y_whole , y_fitted_whole, average = 'binary' )
    presi_b = precision_score(y_whole ,y_fitted_whole, average = 'binary')
    recall_b = recall_score(y_whole , y_fitted_whole, average = 'binary')

    f1_mi = f1_score(y_whole , y_fitted_whole, average = 'micro' )
    presi_mi = precision_score(y_whole ,y_fitted_whole, average = 'micro')
    recall_mi = recall_score(y_whole , y_fitted_whole, average = 'micro') 

    f1_ma = f1_score(y_whole , y_fitted_whole, average = 'macro' )
    presi_ma = precision_score(y_whole ,y_fitted_whole, average = 'macro')
    recall_ma = recall_score(y_whole , y_fitted_whole, average = 'macro')      

    r_w = [f1_w,presi_w,recall_w]
    r_n = [f1_n,presi_n,recall_n]
    r_b = [f1_b,presi_b,recall_b]
    r_mi = [f1_mi,presi_mi,recall_mi]
    r_ma = [f1_ma,presi_ma,recall_ma]
    result = []
    result.append(r_w)
    result.append(r_n)
    result.append(r_b)
...
micro =[]
acc = []
files = glob.glob ('res*/all/*.txt'  )
files.sort()
for f in files:    
    data = pd.read_table(f, sep = ',')
    perc.append(data.iloc[2,2])
    recall.append(data.iloc[2,3])
    f_1.append(data.iloc[2,1])
    acc.append(data.iloc[0,2])
    micro.append(data.iloc[3,1])
